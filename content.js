...
function findLogElements() {
  ...
  for (const selector of selectors) {
    ...
    for (const element of elements) {
      ...
      const hasAnyLogTag = SALESFORCE_LOG_TAGS.some(tag => text.includes(tag));
...
chrome.storage.local.get(['selectedTags', 'enabledTags'], function(result) {
  if (result.enabledTags && result.enabledTags.length > 0) {
    currentEnabledTags = result.enabledTags;
  } else {
    currentEnabledTags = SALESFORCE_LOG_TAGS;
  }
